...
{
    /// <summary>
    /// The settings v1.3.0 and every earlier build read from BepInEx\config\com.headtracking.obradinn.cfg,
    /// with that build's defaults. Frozen: a later change to the runtime settings or to core's
    /// constants must never change what an old .cfg, or one missing a key, reads as. The defaults
    /// are literals for that reason, although the build this was frozen from took the smoothing
    /// pair and the position limits from core, which held these same values.
    /// </summary>
    internal sealed class LegacyConfig
    {
        ...
        public KeyCode ToggleReticleKey = KeyCode.PageDown;

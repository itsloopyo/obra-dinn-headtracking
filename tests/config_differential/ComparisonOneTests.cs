...
{
    /// <summary>
    /// Comparison 1: v1.3.0's own ConfigManager (oracle/ConfigManager.cs, the published file
    /// byte for byte) against the frozen reader in src/ObraDinnHeadTracking/Legacy, on every
    ...
    {
        [Fact]
        public void TheFrozenReaderReadsEveryInputAsV130Did()
        {
            ...
            // BaseUnityPlugin runs, so v1.3.0 never loaded on such a file, and nothing after it
            // does either: the plugin's own code never runs. These are the only such inputs.
            Assert.Equal(
                RefusedByBepInEx(),
                throwing.OrderBy(n => n, StringComparer.Ordinal));
        }
        ...
        public void EveryRecordedFileHoldsTheBytesItsProvenanceNames()
        {
            ...
            string provenance = Path.Combine(Path.Combine(Path.Combine(root, "tests"), "config_differential"), "provenance.tsv");
            int checkedFiles = 0;
            ...
            Assert.True(checkedFiles >= 24, "provenance.tsv names only " + checkedFiles + " repo files");
        }

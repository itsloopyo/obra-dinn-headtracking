...
namespace HeadTracking.Tests
{
    ...
    public class ConfigTests
    {
        ...
        {
            byte[] rendered = ObraDinnConfig.Table().RenderFresh(new RenderHeader(ObraDinnConfig.DisplayName));
        ...
        public void FirstLaunchCreatesTheCommittedBytes()
        {
            ...
            {
                ConfigLoadResult<ObraDinnConfig> loaded = Owner(dir.Path).Load();
        ...
        private static ConfigOwner<ObraDinnConfig> Owner(string dir)
        {
        ...
        private sealed class TempDir : IDisposable
        {
            public TempDir()
            {
                Path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "obradinn-config-" + Guid.NewGuid().ToString("N"));
